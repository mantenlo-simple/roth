...
import com.roth.tags.ui.util.OptionTag;
	...
	*/
	...
	public int doEndTag() throws JspException {
		...
		if (g == null) 
			...
			Boolean htmlCallback = getTypedValue("_htmlCallback", false);
			if (htmlCallback) 
				processHtmlCallback();
			int rowIndex = ((Integer)pageContext.getAttribute("rowIndex")).intValue();
			boolean rowDblClick = getTypedValue("_rowDblClick", false);
			if (rowIndex == -1) 
				g.addButton(generateHtml());
			else if (getValue("iconName").equals("open") || getValue("iconName").equals("view") ||
					 getValue("iconName").equals("edit") || rowDblClick) 
	...
	protected String getButton() {
		String type = getTypedValue("type");
		evalHrefClickMap();
		...
		if (href.equals("#"))
			onclick += returnFalse;
		String caption = getTypedValue("caption");
		String imageName = getTypedValue("iconName");
		String image = imageName == null ? null : 
			           imageName.contains(",") ? Icon.getStackedIcon(imageName.split(",")) : 
	...
	protected void processHtmlCallback() {
		//DataGrid g = (DataGrid)findAncestorWithClass(this, DataGrid.class);
		//Boolean sendParams = getTypedValue("_sendParams", false);
		//if (g != null) setValue("onclick", "Roth.table.htmlAction('" + g.getId() + "', this.href, " + sendParams + "); return false;");
	}
