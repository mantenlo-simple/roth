...
@JdbcTable(schema = "roth", name = "page",
	       primaryKeyColumns = {"book_id", "portlet_id"})
public class PageBean implements Serializable, StateBean, EnhancedBean {
	...
	@JdbcLookup(table = "portlet", key = "portlet_id", value = "portlet_name", valueField = "portletName")
	...
	public boolean equals(Object other) {
		if (other != null && other instanceof PageBean otherPage)
			return Data.eq(bookId, otherPage.bookId) &&
				   Data.eq(portletId, otherPage.portletId) &&
				   Data.eq(pageTitle, otherPage.pageTitle) &&
				   Data.eq(sequence, otherPage.sequence) &&
				   Data.eq(updatedBy, otherPage.updatedBy) &&
				   Data.eq(updatedDts, otherPage.updatedDts);
		else
			return false;
	}
	...
	@Override
	public int hashCode() {
		return Data.complexHashCode(bookId, portletId, pageTitle, sequence, updatedBy, updatedDts);
	}

	@Override
	public EnhancedBean copy() {
		PageBean dest = new PageBean();
    	dest.bookId = bookId;
    	dest.portletId = portletId;
    	dest.pageTitle = pageTitle;
	...
	public void merge(EnhancedBean source) {
		if (source instanceof PageBean sourcePage) {
			bookId = sourcePage.bookId;
			portletId = sourcePage.portletId;
			pageTitle = sourcePage.pageTitle;
			sequence = sourcePage.sequence;
			updatedBy = sourcePage.updatedBy;
			updatedDts = sourcePage.updatedDts;
		}
	}
}
