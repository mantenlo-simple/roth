(function () {
    ...
    */
    ...
    function DTS(elem, config) {
        ...
        if (!elem) {
            throw TypeError("input element or selector required for constructor");
        }
    ...
    DTS.prototype.setup = function () {
        let handler = this.inputElemHandler.bind(this);
        //this.inputElem.addEventListener("focus", handler, false);
        this.inputElem.drop = (e) => { handler(e, this.inputElem); };
        this.inputElem.addEventListener("keydown", handler, false);
        //this.inputElem.addEventListener("blur", handler, false);
    }
    DTS.prototype.inputElemHandler = function (e, inputElem) {
        if (e.type == "keydown") {
        	if (e.keyCode === 32 || (e.altKey && e.keyCode === 40)) {
        		if (!this.dtbox) {
	                this.dtbox = new DTBox(e.target, this);
	            }
	            this.dtbox.visible = true;
	            if (window.calobj)
        			window.calobj.dtbox.visible = false;
        		window.calobj = this;
        		if (!window.calClickHandler)
        			window.calClickHandler = (e) => {
        				if (!window.stopdthide &&
	                        window.calobj &&
        				    !getAncestorWithClass(e.target, "date-selector-wrapper", true) &&
        				    !e.target.parentElement.classList.contains("fa-calendar-days") &&
        				    !e.target.parentElement.classList.contains("cal-time-slider") &&
        				    !e.target.classList.contains("cal-cell") &&
        				    !getAncestorWithClass(e.target, "cal-time", true)) {
        					window.calobj.dtbox.visible = false;
        					window.calobj = undefined;
        				}
        				window.stopdthide = undefined;
        			}; 
        		window.addEventListener("click", window.calClickHandler, false);
        		if (!window.calKeyDownHandler)
        			window.calKeyDownHandler = (e) => {
        				if (window.calobj && e.keyCode === 27) {
        					window.calobj.dtbox.visible = false;
        					window.calobj = undefined;
        				}
        			};
        		window.addEventListener("keydown", window.calKeyDownHandler, false);
        	}
        	...
        	if (this.dtbox && this.dtbox.visible) {
        		...
        		if (!this.dtbox)
        			this.dtbox = new DTBox(inputElem, this);
        		this.dtbox.visible = true;
        		if (window.calobj)
        			window.calobj.dtbox.visible = false;
        		window.calobj = this;
    ...
    DTBox.prototype.setupWrapper = function () {
        ...
        function setPosition(e){
            ...
            var paddingY = config.paddingY === undefined || config.paddingY === null ? 5 : config.paddingY;
            var paddingX = config.paddingX === undefined || config.paddingX === null ? 5 : config.paddingX;
    ...
    DTBox.prototype.setupHeader = function () {
        if (!this.el.header) {
            ...
            row.children[0].innerHTML = '<i class="fas fa-caret-left"></i>';
            row.children[2].innerHTML = '<i class="fas fa-caret-right"></i>';
    ...
    DTBox.prototype.setBodyContent = function () {
        ...
        if ("DAYS" == this.bodyType) {
            ...
            var adjusted = new Date(start.valueOf());
			adjusted.setDate(adjusted.getDate() - start.getDay());
    ...
    DTBox.prototype.setInputValue = function() {
        ...
        if (this.elem.onchange)
	        this.elem.onchange();
    }

    DTBox.prototype.onDateSelected = function (e) {
        let row = e.target.parentNode;
        let date = parseInt(e.target.innerText);
        let direct = true;
    ...
    DTBox.prototype.onHeaderChange = function (e) {
        var cell = e.target;
		while (cell && !cell.classList.contains('cal-nav'))
        ...
        if (cell.previousSibling && cell.nextSibling) {
			...
			var sign = cell.classList.contains('cal-nav-next') ? 1 : -1;
