 ...
 */
SELECT g.group_id,
       CONCAT(REPEAT('- ', LENGTH(TRIM(g.lineage)) - LENGTH(TRIM(REPLACE(g.lineage, ' ', '')))), g.group_name) AS group_name
  FROM (SELECT gx.group_id,
               gx.group_name,
               gx.parent_group_id,
               gx.lineage
          FROM `group` gx
           ...
           AND group_id IN (SELECT group_id
                             ...
                             WHERE EXISTS (SELECT g.lineage
                                            ...
                                            WHERE g.group_id = r.group_id
                                              ...
                                              AND r.role_name = 'GroupNewsAdmin'
                                              AND u.userid = {1}
                                              AND u.domain_id = {2}
       ...
       ) g LEFT OUTER JOIN user_group u
